...
from zohosdk.src.com.zoho.officeintegrator.v1 import InvalidConfigurationException, CreatePresentationParameters, \
    SessionDeleteSuccessResponse, FileDeleteSuccessResponse
...
class DeletePresentation:
    ...
    def execute():
        ...
        print('Creating a presentation to demonstrate presentation delete api')
        ...
        if response is not None:
            ...
            if responseObject is not None:
                if isinstance(responseObject, CreateDocumentResponse):
                    presentationId = str(responseObject.get_document_id())
                    print('Presentation Session ID to be deleted : ' + presentationId)

                    deleteApiResponse = v1Operations.delete_presentation(presentationId)
                    ...
                    if deleteApiResponse is not None:
                        ...
                        if deleteResponseObject is not None:
                            if isinstance(deleteResponseObject, FileDeleteSuccessResponse):
                                print('Presentation delete status : ' + str(deleteResponseObject.get_doc_delete()))
